...
void addGuess(MastermindWindow& mwin, const string code, const int size, const char startLetter, const int round)
{
	...
	for (int i = 0; i < size; ++i) {
		...
		mwin.vr.push_back(new Rectangle{ Point{tlDispOneColorX, tlDispOneColorY}, static_cast<int>(oneColor_w * mwin.x_max()),
...
void hideCode(MastermindWindow& mwin, int size)
{
	...
	mwin.vr.push_back(
		new Rectangle{ Point{static_cast<int>(roomAroundGuess * mwin.x_max()), static_cast<int>(vRoomAboveGuess * mwin.y_max())},
...
{
	const int centerY =25+ 50*round; // Regn ut sirklenes y-koordinat (sentrum)
	for (int i = 0; i < size; ++i) {
		const int centerX = 300+20*i; // Regn ut sirkelens x-koordinat (sentrum)

		mwin.vc.push_back(new Circle{ Point{centerX, centerY}, 8 });

		if (i < correctPosition) {
				mwin.vc[i].set_fill_color(Color::black);
			// Sett farge for korrekt plassering
		}
		else if (i < (correctPosition + max(0, correctCharacter - correctPosition))) {
				mwin.vc[i].set_fill_color(Color::white);
			// Sett farge for korrekt bokstav, men feil plassering
		}

		mwin.attach(mwin.vc[mwin.vc.size()-1]);// Koble sirkelen til vinduet (attach)
